...
export default function LoginPage() {
	...
	const mintNFT = async () => {
		try {
			...
			// @ts-ignore
			bastionConnect.init(ethersProvider, { privateKey: process.env.NEXT_PUBLIC_PRIVATE_KEY as string, rpcUrl: process.env.NEXT_PUBLIC_RPC_URL1 as string, chainId: 80001 });
			...
			const res = await nftContract.safeMint("0x2429EB38cB9b456160937e11aefc80879a2d2712");
			console.log("res", res);
		} catch (e) {
			console.error(e);
		}
		...
		<div className="flex flex-col items-center min-h-screen bg-gray-100 py-2">
			<div className="p-6 max-w-lg w-full bg-white shadow-md rounded-md mt-20">
				...
				{ethersProvider ? (
					<div className="mt-4">
						Hello {address}
						<button onClick={() => mintNFT()} className="py-2 px-4 mt-4 w-full text-center bg-red-600 rounded-md text-white text-sm hover:bg-red-500">
							Mint NFT
						</button>
					</div>
				) : (
					<div className="mt-4">
						<button onClick={() => loginWithProvider()} className="py-2 px-4 w-full text-center bg-red-600 rounded-md text-white text-sm hover:bg-red-500">
							Particle Auth
						</button>
					</div>
				)}
